...
static int sock;
static void
msgwalengine(guint64 pageid, const char *s)
{
...
pageusermessagereply(GObject *o, GAsyncResult *r, gpointer page)
{
	...
	if (strcmp(name, "walengine-pipe") != 0) {
		fprintf(stderr, "webext-walengine: Unknown User Reply: %s\n", name);
		return;
	}
	gfd = webkit_user_message_get_fd_list(m);
	if ((nfd = g_unix_fd_list_get_length(gfd)) != 1) {
		fprintf(stderr, "webext-walengine: Too many file-descriptors: %d\n", nfd);
		return;
	}
